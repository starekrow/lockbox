...
class KeyDrop
{
    ...
    /**
     * Determine whether a given named field in a query matches a field 
     * specifier in a policy.
     *
     */
    public function checkPolicyField($name, $q, $p)
    {
        if (!isset($p->$name)) {
            return true;
        }
        ...
        $qf = $q->$name;
        $pregtr = [ '\*' => '.*', '\?' => '.', '\[' => '[', '\]' => ']' ];
        $pfl = is_array($p->$name) ? $p->$name : [ $p->$name ];
        foreach ($pfl as $pf) {
            $pattern = strtr(preg_quote($pf, '#'), $pregtr);
            if (preg_match( "#^" . $pattern . "$#", $qf )) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Verify that the query is allowed.
     *
     */
    public function checkAllowed($q, $source)
    {
        $pl = $this->dumpValues("policy", null, $this->agentId);
        $pl = array_values( (array)$pl );
        if (empty($q->action)) {
            return false;
        }
        $allow = false;
        for ($i = 0; $i < count($pl); ++$i) {
            if ($i > 1000) {
                error_log( "Cycle in KeyDrop::checkAllowed()" );
                break;
            }
            if (is_string( $p )) {
                $xpl = $this->getClientValue($p,"%policyGroups");
                $pl = array_merge($pl, $xpl);
                continue;
            }
            if (!checkPolicyField("action", $q, $p)
                || !checkPolicyField("source", $q, $p)
                || !checkPolicyField("forClient", $q, $p)
                || !checkPolicyField("type", $q, $p)
                || !checkPolicyField("name", $q, $p)
                || !checkPolicyField("value", $q, $p)) {
                continue;
            }
     ...
     * @param string $source Source of query (e.g. IP address)
     * @return false|array Response to query
     */
    public function handleQuery($query, $source)
    {
        $q = explode("|", $query, 3);
        if (count($q) != 3 || $q[0] != "q1") {
            return false;
        ...
        $client = $q[1];
        $this->setAgentId($client);
        $this->setClientId($client);
        $mkey = CryptoKey::import($this->getClientValue("messageKey"));
        if (!$mkey) {
            return false;
        }
        $q = $mkey->unlock($q[2]);
        ...
        if (isset($q->forClient)) {
            $this->setClientId($q->forClient);
        }
        if (is_array( $q->batch )) {
            $ql = $q->batch;
            if (!count( $ql )) {
        ...
        foreach ($ql as $cmd) {
            if (!$this->checkAllowed($cmd, $source)) {
                return false;
            }            
        }
        $res = [];
        foreach ($ql as $cmd) {
            ...
            switch ($cmd->action) {
                case "get":
                    $res = $this->getValue($cid, $cmd->type, $cmd->name);
                    break;
                case "set":
                    $res = $this->setValue($cid, $cmd->type, $cmd->name, $cmd->value);
                    break;
                case "enum":
                    $name = isset($cmd->name) ? $cmd->name : null;
                    $res = $this->enumValues($cid, $cmd->type, $name);
                    break;
                case "dump":
                    $name = isset($cmd->name) ? $cmd->name : null;
                    $res = $this->dumpValues($cid, $cmd->type, $name);
                    break;
                //case "config":
                //    $res = $this->configClient($cmd->name, $cmd->value);
                //    break;
                case "newClient":
                    $res = $this->createClient($cmd->name);
                    break;
                case "delClient":
                    $res = $this->deleteClient($cmd->name);
                    break;
                default:
                    return false;
            }
        ...
        $this->autosaveClients();
        return "r1|" . $mkey->lock(json_encode($res));
    }

    /**
     * Creates, updates or removes a value for the current client. If `$name`
     * ends with "-#", the "#" is replaced with a numeric counter value that
     * will remain unique within this client.
     *
     * @param string $client Client ID to affect
     * @param string $type Type of value
     * @param string $name Name for this value
     * @param string|null $val Value to set, or `null` to remove
     */
    public function setValue($client, $type, $name, $val)
    {
        $ci = $this->clientInfo($client);
        if (!isset($ci->$type)) {
            $ci->$type = new stdClass();
        }
        $res = true;
        if (strlen($name) > 2 && substr($name, -2, 2) == "-#") {
            $name = substr($name, 0, -1) . $ci->autoCounter++;
            $res = $name;
        }
        if ($val === null) {
            unset( $ci->$type->$name );
    ...
    /**
     * Reads a value from the current client.
     *
     * @param string $client Client ID to read
     * @param string $type Type of value
     * @param string $name Name of the value
     * @return mixed The value in question, or `null` if not found
     */
    public function getValue($client, $type, $name)
    {
        $ci = $this->clientInfo($client);
        if (!isset($ci->$type) || !isset($ci->$type->$name)) {
            return null;
        }
        return $ci->$type->$name;
    }
    ...
    /**
     * Gets a list of named values matching the given criteria.
     *
     * @param string $client Client ID to read
     * @param string $type Type of value
     * @param string $name Prefix to filter by (or `null`)
     * @return array List of value names
     */
    public function enumValues($client, $type, $name)
    {
        $ci = $this->clientInfo($client);
        $out = [];
        if (isset($ci->$type)) {
            $nl = isset($name) ? strlen($name) : 0;
            foreach ($ci->$type as $k => $v) {
                if ($nl && $name != substr($k, 0, $nl)) {
                    continue;
                }
                $out[] = $k;
            }
    ...
    /**
     * Returns a set of named values matching the given criteria.
     *
     * @param string $client Client ID to read
     * @param string $type Type of value
     * @param string $name Prefix to filter by (or `null`)
     * @return object Map of value names to values
     */
    public function dumpValues($client, $type, $name)
    {
        $ci = $this->clientInfo($client);
        $out = new stdClass();
        if (isset($ci->$type)) {
    ...
    {
        if (isset($this->clientCache[$client])) {
            return $this->clientCache[$client];
        }
        $fn = $this->dataDir . "/$name.json";
        if (!file_exists($fn)) {
            return false;
        }
        $ci = json_decode(file_get_contents($fn));
        return ($this->clientCache[$client] = $ci);
    }

    /**
     *
     */
    public function autosaveClients($client)
    {
        foreach ($this->clientCache as $k => $v) {
            if (!$v->dirty) {
                continue;
            }
            unset($v->dirty);
            $fn = $this->dataDir . "/$k.json";
    ...
    public function createClient($name)
    {
        ...
        file_put_contents($fn, json_encode($ci));
        $this->clientCache[$name] = $ci;
        $this->clientId = $name;
    }
     ...
     *
     */
    public static function encodeQuery($clientId, $query, $messageKey)
    {
     ...
     *
     */
    public static function decodeResponse($response, $messageKey)
    {
